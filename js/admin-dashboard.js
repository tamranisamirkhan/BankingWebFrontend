...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function loadCustomers() {
    ...
    try {
      const url = `http://51.20.82.164:8080/smartBank/admin/getAllCustomer?status=${currentFilter}`;
  ...
  async function updateStatus(id, action) {
    ...
    try {
      const res = await fetch(`http://51.20.82.164:8080/smartBank/admin/${endpoint}`, { method, credentials: "include" });
